...
int main() {
  ...
  cin>>h>>w;
  vector<string> v(h);
  vector<vector<ll>> vo(h,vector<ll>(w,0));
  vector<vector<ll>> vi(h,vector<ll>(w,0));
  rep(i,h) cin>>v[i];
  rep(i,h){
    for(ll j=w-1;j>=0;j--){
      if(v[i][j]=='O') vo[i][j]++;
      if(j!=w-1) vo[i][j]+=vo[i][j+1];
    }
  }
  rep(i,w){
    for(ll j=h-1;j>=0;j--){
      if(v[j][i]=='I') vi[j][i]++;
      if(j!=h-1) vi[j][i]+=vi[j+1][i];
    }
  }
  ll ans=0;
  rep(i,h) rep(j,w) if(v[i][j]=='J') ans+=vi[i][j]*vo[i][j];
  cout<<ans<<endl;
}