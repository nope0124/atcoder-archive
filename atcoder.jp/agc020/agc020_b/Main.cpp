...
int main() {
  ...
  ll minNG = 0;
  ll maxNG = 1e16;
  while (abs(maxOK - maxNG) > 1) {
    ll mid = (maxOK + maxNG) / 2;
    if (solve(mid) == 2) maxOK = mid;
    else maxNG = mid;
  }
  while (abs(minOK - minNG) > 1) {
    ll mid = (minOK + minNG) / 2;
    if (solve(mid) == 2) minOK = mid;
    else minNG = mid;
  }
  cout << minOK << ' ' << maxOK << endl;
}