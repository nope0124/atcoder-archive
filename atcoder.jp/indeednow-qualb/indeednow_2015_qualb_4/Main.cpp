...
int main() {
  ...
  rep (i, n) cin >> v[i], v[i]--;
  vector<ll> color[c];
  rep (i, c) color[i].push_back(0), color[i].push_back(n + 1);
  rep (i, n) color[v[i]].push_back(i + 1);
  rep (i, c) sort(ALL(color[i]));
  rep (i, c) {
    ll ans = n * (n + 1) / 2;
    rep (j, color[i].size() - 1) {
      ll tmp = color[i][j + 1] - color[i][j];
      ans -= tmp * (tmp - 1) / 2;
    }
    cout << ans << endl;
  }
