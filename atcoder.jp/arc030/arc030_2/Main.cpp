...
vector<ll> g[110];
vector<bool> flag(5100,false);
void dfs(ll s,ll t,vector<ll> f,ll r){
  for(auto c:g[s]){
    if(r==c) continue;
    if(c==t){
      f.push_back(t);
      Rep(i,f.size()) flag[max(f[i-1],f[i])*(max(f[i-1],f[i])-1)/2+min(f[i-1],f[i])]=true;
      return;
    }
    vector<ll> tf=f;
    tf.push_back(c);
