...
vector<bool> reached(1000010,false);
const int MAX_N=1000010;
vector<bool> sieve_of_eratosthenes(vector<ll> &v,ll n){
  vector<bool> isPrime(MAX_N+1,true);
  rep(i,n){
    if(isPrime[v[i]]){
      if(reached[v[i]]) isPrime[v[i]]=false;
      reached[v[i]]=true;
      for(int j=2*v[i];j<=MAX_N;j+=v[i]){
        isPrime[j]=false;
      }
    }
  }
...
int main() {
  ...
  rep(i,n) cin>>v[i];
  sort(ALL(v));
  vector<bool> tmp=sieve_of_eratosthenes(v,n);
  ll ans=0;
  rep(i,n){
    if(tmp[v[i]]) ans++;
  }
