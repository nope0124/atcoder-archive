...
int main() {
  ...
  vector<ll> acc_max(n + 1, 0);
  rep (i, n) {
    if (i == 0) acc_max[i] = sum[i];
    else acc_max[i] = max(acc_max[i - 1], sum[i]);
  }
  ll ans = 0;
  vector<ll> two_sum(n + 1);
  rep (i, n + 1) {
    if (i == 0) two_sum[i] = sum[i];
    else two_sum[i] = two_sum[i - 1] + sum[i];
  }
  rep (i, n) {
    chmax(ans, two_sum[i + 1] + acc_max[i + 1]);
  }
  cout << ans << endl;
}