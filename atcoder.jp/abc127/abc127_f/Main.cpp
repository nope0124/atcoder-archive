...
int main() {
  IOS;
  ll q;cin>>q;
  priority_queue<ll> zenhan;
  priority_queue<ll,vector<ll>,greater<ll>> kouhan;
  ll zensum=0,kousum=0,sum=0;
  rep(i,q){
    ll type;cin>>type;
    if(type==1){
      ll a,b;cin>>a>>b;
      sum+=b;
      if(zenhan.size()==kouhan.size()){
        //前半に
        if(zenhan.empty()){
          zenhan.push(a);
          zensum+=a;
      ...
      ll x=zenhan.top();
      ll y=sum+(zenhan.size()*x-zensum)+(kousum-kouhan.size()*x);
      cout<<x<<' '<<y<<endl;
    }
  }
}