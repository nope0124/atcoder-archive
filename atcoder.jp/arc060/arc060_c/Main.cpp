...
vvl doubling(30, vl(100010, 0));

ll solve(ll mid, ll a) {
  rep (i, 30) if (mid & (1LL << i)) a = doubling[i][a];
  return a;
}
...
int main() {
  ...
  rep (i, N) cin >> V[i];
  ll L, Q; cin >> L >> Q;
  rep (i, N) doubling[0][i] = upper_bound(ALL(V), V[i] + L) - V.begin() - 1;
  rep (d, 29) rep (i, N) doubling[d + 1][i] = doubling[d][doubling[d][i]];
  while (Q--) {
    ll a, b; cin >> a >> b; a--; b--;
    if (a > b) swap(a, b);
    ll ok = 2e9;
