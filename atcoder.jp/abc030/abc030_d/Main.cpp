...
vector<ll> BASE(string s, ll p) {
  ll n = s.size();
  reverse(ALL(s));
  vector<ll> base(n);
  rep (i, n) {
    if (i == 0) base[i] = 1;
    else base[i] = (base[i - 1] * 10) % p;
  }
  vector<ll> ret(n);
  rep (i, n) {
    if (i == 0) ret[i] = (s[i] - '0') % p;
    else ret[i] = (ret[i - 1] + (s[i] - '0') * base[i]) % p;
  }
  return ret;
}
...
int main() {
  ...
  if (k.size() <= 6) {
    len = 0;
    rep (i, k.size()) {
      len += POW(10, k.size() - i - 1) * (k[i] - '0');
    }
  }
  vector<ll> v(n);
  rep (i, n) cin >> v[i], v[i]--;
  
  ll loopstart = get_loopstart(v, a);
  vector<ll> preloop = get_preloop(v, a, loopstart);
  vector<ll> loop = get_loop(v, loopstart);
  
  ll S= preloop.size();
  if (len >= S) {
    len -= S;
    cout << loop[len % loop.size()] + 1 << endl;
    return 0;
  } else if (len != -1) {
    cout << preloop[len] + 1 << endl;
    return 0;
  }
  ll p = loop.size();
  n = k.size();
  
  vector<ll> w = BASE(k, p);
  
  w[n - 1] -= S;
  while (w[n - 1] < 0) {
    w[n - 1] += p;
  }
  cout << loop[w[n - 1] % p] + 1 << endl;
}