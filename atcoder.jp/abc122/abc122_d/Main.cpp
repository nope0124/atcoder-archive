...
int main() {
  ...
  cin>>n;
  ll dp[n][5][5][5];
  rep(i,n) rep(j,5) rep(k,5) rep(l,5) dp[i][j][k][l]=0;
  Rep(i,5) dp[0][i][0][0]=1;
  rep(i,n-1) Rep(j,5) Rep(k,5) rep(l,5) rep(m,5){
    if(j==2 && k==3 && l==1) continue;
    if(j==2 && k==1 && l==3) continue;
    if(j==3 && k==2 && l==1) continue;
    if(j==2 && l==3 && m==1) continue;
    if(j==2 && k==3 && m==1) continue;
