...
ll n;
void dfs(ll v, ll c, ll edgec) {
  if (reached[v] != -1) return;
  ll cur = -1;
  for (auto tmp : color[v]) {
    if (c != tmp) {
      cur = tmp;
      break;
    }
  }
  if (c != edgec) cur = edgec;
  if (cur == -1) {
    cur = (c + 1) % n;
  }
  reached[v] = cur;
  for (auto tmp : graph[v]) {
    dfs(tmp.to, cur, tmp.cost);
  }
}
