...
int main() {
  ...
  if(n%2==0){
    cout<<n/2<<endl;
    rep(i,n/2){
      cout<<2<<' '<<v[i]<<' '<<v[n-i-1]<<endl;
    }
    return 0;
  }
  cout<<cnt<<endl;
  bool flag=true;
  vector<vector<ll>> ans(cnt,vector<ll>(n/cnt,0));
  ll ind=0;
  rep(i,n-2*cnt){
    ans[ind][i/cnt]=v[i];
    if(flag){
  ...
  ll tmp=n/cnt;
  ans[cnt/2][tmp-2]=sv[cnt-1];
  ans[cnt/2][tmp-1]=lv[0];
  rep(i,cnt/2){
    ans[i][tmp-2]=sv[cnt-2-2*i];
    ans[cnt-1-i][tmp-2]=sv[i*2];
  }

  rep(i,cnt/2){
    ans[i][tmp-1]=lv[cnt/2+1+i];
    ans[i+cnt/2+1][tmp-1]=lv[i+1];
  }
  rep(i,cnt){
    cout<<n/cnt;
    rep(j,n/cnt){
