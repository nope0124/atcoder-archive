...
int main() {
  ...
  vector<vector<ll>> sum(h,vector<ll>(w,0));
  rep(i,h) rep(j,w) if(v[i][j]=='.') sum[i][j]=left[i][j]+right[i][j]+up[i][j]+down[i][j]-3;
  /*rep(i,h){
    rep(j,w){
      cout<<sum[i][j]<<' ';
    }
  ...
  mint ans=0;
  mint tmp=modpow(2,cnt,MOD);
  rep(i,h) rep(j,w){
    mint ntmp=tmp-modpow(2,cnt-sum[i][j],MOD);
    ans+=ntmp;
  }
  cout<<ans<<endl;
}