...
int main() {
  ...
  vector<string> v(h);
  rep(i,h) cin>>v[i];
  vector<vector<ll>> value(h,vector<ll>(w,-1));
  queue<pint> que;
  rep(i,h) rep(j,w) if(v[i][j]=='#') que.push(pint(j,i)),value[i][j]=0;
  while(!que.empty()){
    ...
    rep(i,4){
      ...
      value[ny][nx]=value[c.second][c.first]+1;
      que.push(pint(nx,ny));
    }
  }
  ll ans=0;
  rep(i,h) rep(j,w) chmax(ans,value[i][j]);
  cout<<ans<<endl;
}