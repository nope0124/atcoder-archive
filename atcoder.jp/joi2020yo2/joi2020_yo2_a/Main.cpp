...
int main() {
  ...
  ll n,ans=1e16;
  cin>>n;
  vector<string> s(n),t(n),t1(n),t2(n),t3(n);
  rep(i,n) cin>>s[i];
  rep(i,n) cin>>t[i];
  rep(i,n){
    string tmp;
  ...
  rep(_,4){
    if(_==0){
      ll tmp=0;
      rep(i,n) rep(j,n) if(t[i][j]!=s[i][j]) tmp++;
      chmin(ans,tmp);
    }else if(_==1){
      ll tmp=1;
      rep(i,n) rep(j,n) if(t[i][j]!=t1[i][j]) tmp++;
      chmin(ans,tmp);
    }else if(_==2){
      ll tmp=2;
      rep(i,n) rep(j,n) if(t[i][j]!=t2[i][j]) tmp++;
      chmin(ans,tmp);
    }else if(_==3){
      ll tmp=1;
      rep(i,n) rep(j,n) if(t[i][j]!=t3[i][j]) tmp++;
      chmin(ans,tmp);
    }
  }
  cout<<ans<<endl;
