...
int main() {
  ...
  rep(i,n) {
    for(ll ii = i+ 1;ii<n;ii++){
      bool flag= true;
      rep(j,n) {
        if(v[i][j] + v[ii][j] >k) flag=false;
      }
      if(flag){
        p1.push_back(pint(i,ii));
      }
    }
  }
  rep(i,n) {
    for(ll ii = i+ 1;ii<n;ii++){
      bool flag= true;
      rep(j,n) {
        if(v[j][i] + v[j][ii] >k) flag=false;
      }
      if(flag){
        p2.push_back(pint(i,ii));
      }
    }
  }
  UnionFind uf1(n),uf2(n);
  rep(i,p1.size()) {
    uf1.merge(p1[i].first,p1[i].second);
  }
  rep(i,p2.size()){
    uf2.merge(p2[i].first,p2[i].second);
  }
  mint ans = 1;
  vector<bool> flag1(n,false),flag2(n,false);
  rep(i,n){
    if(!flag1[uf1.root(i)]){
      rep(j,uf1.size(i)){
        ans *= j+1;
      }
      flag1[uf1.root(i)]=true;
    }
  }
  rep(i,n){
    if(!flag2[uf2.root(i)]){
