...
double p;
double f(double x){return x+p*(double)pow(2,(double)(-x/1.5));}

bool solve(double x,double y){return (f(x)>=f(y)) ? true : false;}
/*↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
int main() {
  IOS;
  cin>>p;
  ...
  while(abs(ok-ng)>0.00000000001){
    double left=(double)(2*ok+ng)/3;
    double right=(double)(ok+ng*2)/3;
    if(solve(left,right)) ok=left;
    else ng=right;
  }
