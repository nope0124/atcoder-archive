...
int main() {
  ...
  ll ans=1e16;
  vector<ll> dp(n,0);
  vector<string> v={"AA","BB","XX","YY","AB","BA","AX","XA","AY","YA","BX","XB","BY","YB","XY","YX"};
  rep(i,16){
    for(ll j=i+1;j<16;j++){
      rep(k,n) dp[k]=k+1;
      bool flag=true;
      ...
      rep(k,n-1){
        ...
        tmp+=s[k];
        tmp+=s[k+1];
        if(v[i]==tmp && flag && (!reached[k] && !reached[k+1])){
          flag=false;
          reached[k]=true;
          reached[k+1]=true;
          for(ll l=k+1;l<n;l++) dp[l]--;
        }else{
          flag=true;
        }
      }
      ...
      rep(k,n-1){
        ...
        tmp+=s[k];
        tmp+=s[k+1];
        if(v[j]==tmp && flag && (!reached[k] && !reached[k+1])){
          reached[k]=true;
          reached[k+1]=true;
          flag=false;
          for(ll l=k+1;l<n;l++) dp[l]--;
