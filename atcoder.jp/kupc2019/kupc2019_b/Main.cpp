...
int main() {
  ...
  map<ll,ll> mpv;
  rep(i,n){
    mpw[uf.root(i)]+=weight[i];
    mpv[uf.root(i)]+=value[i];
  }
  vector<ll> w,v;
  for(auto c:mpw) w.push_back(c.second);
  for(auto c:mpv) v.push_back(c.second);
  ll dp[w.size()+1][W+1];
  rep(i,w.size()+1) rep(j,W+1) dp[i][j]=0;
  rep(i,w.size()){
    rep(j,W+1){
  ...
  ll ans=0;
  rep(i,W+1) chmax(ans,dp[w.size()][i]);
  cout<<ans<<endl;
}