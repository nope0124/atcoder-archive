...
struct edge{ll fx,fy,tx,ty,cost;};
/*↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
int main() {
  IOS;
  ll h,w;cin>>h>>w;
  ll sy,sx,gy,gx;cin>>sy>>sx>>gy>>gx;sx--;sy--;gx--;gy--;
  vector<string> v(h);
  rep(i,h) cin>>v[i];
  ll dx[4]={-1,1,0,0};
  ll dy[4]={0,0,-1,1};
  ...
  deque<edge> que;
  que.push_front(edge{sx,sy,sx,sy,0});
  bool flag=true;
  while(!que.empty()){
    ...
    if(flag){
      ...
      if(dist[ty][tx]!=1e16) continue;
      dist[ty][tx]=dist[fy][fx]+cost;
    }
    rep(i,4){
      ll nx=tx+dx[i];
      ll ny=ty+dy[i];
      ...
      if(v[ny][nx]=='#') continue;
      if(dist[ny][nx]!=1e16) continue;
      que.push_front(edge{tx,ty,nx,ny,0});
    }
    rep(i,4){
      rep(j,4){
        ll nx=tx+d[i];
        ...
        if(v[ny][nx]=='#') continue;
        if(dist[ny][nx]!=1e16) continue;
        que.push_back(edge{tx,ty,nx,ny,1});
      }
    }
    rep(i,4){
      ll nx=tx+2*dx[i];
  ...
  if(dist[gy][gx]==1e16) cout<<-1<<endl;
