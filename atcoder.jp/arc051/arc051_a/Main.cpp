...
ll x,y,r;
bool inCircle(ll rx,ll ry){
  bool ans=false;
  if((x-rx)*(x-rx)+(y-ry)*(y-ry)<=r*r) ans=true;
  return ans;
}
/*↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
int main() {
  IOS;
  ll lrx,rrx,lry,rry;
  cin>>x>>y>>r>>lrx>>lry>>rrx>>rry;
  ll rflag=true;
  ll bflag=true;
  if(inCircle(lrx,lry) && inCircle(lrx,rry) && inCircle(rrx,lry) && inCircle(rrx,rry)) bflag=false;
  if(y-r>=lry && y+r<=rry && x-r>=lrx && x+r<=rrx) rflag=false;
  if(rflag) cout<<"YES"<<endl;
  else cout<<"NO"<<endl;
