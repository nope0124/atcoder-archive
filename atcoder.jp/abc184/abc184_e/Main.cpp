  ...
  rep (i, h) {
    rep (j,w) {
      rep (k, 4) {
        ...
        if (v[ny][nx] == '#') continue;
        graph[ny * w + nx].push_back(edge{i * w + j, 1});
        graph[i * w + j].push_back(edge{ny * w + nx, 1});
        if (v[i][j] >= 'a' && v[i][j] <= 'z') {
          graph[i * w + j].push_back(edge{4000000 + v[i][j] - 'a', 1});
          graph[4000000 + v[i][j] - 'a'].push_back(edge{i * w + j, 0});
        }
      }
    }
  }
