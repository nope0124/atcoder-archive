...
int main() {
  ...
  rep(i,n+1) rep(j,n+1) v[i][j+1]=v[i][j]+v[i][j+1];
  rep(i,n+1) rep(j,n+1) v[i+1][j]=v[i][j]+v[i+1][j];
  rep(i,n+1) rep(j,n+1) v[i+1][j+1]=v[i][j]+v[i+1][j+1];
  rep(i,n+2) rep(j,n+2) if(v[i][j]>=1) ans++;
  cout<<ans<<endl;
}