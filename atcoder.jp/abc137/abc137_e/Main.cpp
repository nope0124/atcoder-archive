...
ll d[5010];
ll V,E;
vector<bool> negative(5010,false);
...
int main() {
  ...
  ll p;
  cin>>V>>E>>p;
  vector<ll> g1[5010],g2[5010];
  rep(i,E){
    ll a,b,c;cin>>a>>b>>c;a--;b--;c*=-1;c+=p;
    graph[i]=edge{a,b,c};
    g1[a].push_back(b);
    g2[b].push_back(a);
  }
  vector<bool> r1(5010,false),r2(5010,false);
  queue<ll> que;
  r1[0]=true;
  que.push(0);
  while(!que.empty()){
  ...
  bool flag=true;
  bellman_ford(0);
  rep(i,5010) if(r1[i] && r2[i] && negative[i]) flag=false;
  if(!flag) cout<<-1<<endl;
  else cout<<max((ll)0,-1*d[V-1])<<endl;
}