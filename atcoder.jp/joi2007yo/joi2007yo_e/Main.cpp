...
int main() {
  ...
  vector<group> w;
  rep(i,n){
    ll p,q,r,s;cin>>p>>q>>r>>s;p--;q--;r--;
    if(s==1) v[p]=v[q]=v[r]=true;
    else w.push_back(group{p,q,r});
  }
  vector<ll> ans(a+b+c,2);
  rep(i,a+b+c) if(v[i]) ans[i]=1;
  rep(i,w.size()){
    vector<ll> tmp;
    if(!v[w[i].a]) tmp.push_back(w[i].a);
    if(!v[w[i].b]) tmp.push_back(w[i].b);
    if(!v[w[i].c]) tmp.push_back(w[i].c);
    if(tmp.size()==1) ans[tmp[0]]=0;
  }
