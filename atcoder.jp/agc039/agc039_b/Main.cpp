...
vector<ll> graph[210];
vector<ll> color(210, 0);

bool bipartite(ll v, ll c) {
  color[v] = c;
  rep (i, graph[v].size()) {
    if (color[graph[v][i]] == c) return false;
    if (color[graph[v][i]] == 0 && !bipartite(graph[v][i], -c)) return false;
  }
  return true;
}
...
int main() {
  ...
  rep (i, n) dist[i][i] = 0;
  rep (i, n) rep (j, n) {
    if (v[i][j] == '1') dist[i][j] = 1;
  }
  rep (k, n) rep (i, n) rep (j, n) chmin(dist[i][j], dist[i][k] + dist[k][j]);
  ll ans = 0;
  rep (i, n) rep (j, n) if (i != j && dist[i][j] != 1e16) chmax(ans, dist[i][j]);
  if (bipartite(0, 1)) cout << ans + 1 << endl;
  else cout << -1 << endl;
}