...
int main() {
  ...
  vector<vector<ll>> memot(n,vector<ll>(m,1e16));
  vector<vector<ll>> mis(n,vector<ll>(m,0));
  rep(i,y){
    ll a,b,c;
    string s;
    cin>>a>>b>>c>>s;b--;c--;
    if(s=="open"){
      if(memot[b][c]==1e16) memot[b][c]=a;
    }else if(s=="correct"){
      memo[b][c]=max(p[c]-(a-memot[b][c])-120*mis[b][c],x);
    }else{
      mis[b][c]++;
    }
  }
