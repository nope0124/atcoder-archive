...
struct tri{ll a;string b;ll c;};
bool cmp(tri a, tri b) { return a.b < b.b; }
bool cmp1(tri a, tri b) { return a.c > b.c; }
/*↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
int main() {
  IOS;
  ll n;cin>>n;
  ...
  rep(i,n){
    ...
    p[i].second*=-1;
    mp[pair<string,ll>(p[i].first,p[i].second)]=i+1;
  }
  sort(ALL(p));
  rep(i,n) cout<<mp[pair<string,ll>(p[i].first,p[i].second)]<<endl;
}