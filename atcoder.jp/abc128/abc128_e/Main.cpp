...
int main() {
  ...
  rep (i, n) {
    ll s, t, x; cin >> s >> t >> x;
    v.push_back(pair<ll, pint>(s - x, pint(-1, x)));
    v.push_back(pair<ll, pint>(t - x, pint(0, x)));
  }
  rep (i, q) {
    ll d; cin >> d;
    v.push_back(pair<ll, pint>(d, pint(1, i)));
  }
  sort(ALL(v));
  multiset<ll> m;
  vector<ll> ans(q);
  for (auto c: v) {
    ll type = c.second.first;
    ll value = c.second.second;
    if (type == -1) m.insert(value);
    else if (type == 0) m.erase(m.lower_bound(value));
    else {
      if (m.empty()) ans[value] = -1;
      else ans[value] = *(m.begin());
    }
  }
  dis(ans);
}