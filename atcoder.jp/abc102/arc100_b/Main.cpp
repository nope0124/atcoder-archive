...
int main() {
  ...
  vector<ll> v(n);
  rep (i, n) cin >> v[i];
  vector<ll> l = acc_sum(v);
  reverse(ALL(v));
  vector<ll> r = acc_sum(v);
  ll tmp = n - 3;
  ll ans = 1e16;
  Rep (i, tmp + 1) {
    ll lb = l[i];
    ll rb = r[tmp - i + 1];
    ll lind = upper_bound(ALL(l), lb / 2) - l.begin() - 1;
    ll rind = upper_bound(ALL(r), rb / 2) - r.begin() - 1;
    if (lind >= 0 && rind >= 0) {
      chmin(ans, max({lb - l[lind], l[lind], rb - r[rind], r[rind]}) - min({lb - l[lind], l[lind], rb - r[rind], r[rind]}));
    }
    if (lind + 1 < i && rind >= 0) {
      chmin(ans, max({lb - l[lind + 1], l[lind + 1], rb - r[rind], r[rind]}) - min({lb - l[lind + 1], l[lind + 1], rb - r[rind], r[rind]}));
    }
    if (lind >= 0 && rind + 1 < tmp - i + 1) {
      chmin(ans, max({lb - l[lind], l[lind], rb - r[rind + 1], r[rind + 1]}) - min({lb - l[lind], l[lind], rb - r[rind + 1], r[rind + 1]}));
    }
    if (lind + 1 < i && rind + 1 < tmp - i + 1) {
      chmin(ans, max({lb - l[lind + 1], l[lind + 1], rb - r[rind + 1], r[rind + 1]}) - min({lb - l[lind + 1], l[lind + 1], rb - r[rind + 1], r[rind + 1]}));
    }
  }
  cout << ans << endl;
}