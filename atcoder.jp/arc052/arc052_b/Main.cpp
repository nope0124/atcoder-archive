...
int main() {
  ...
  rep (i, q) {
    ...
    ld ans = 0;
    rep (j, n) {
      ld tmp = (PI * r[j] * r[j] * h[j]) / 3;
      ll A = x[j] + h[j] - a;
      ll B = x[j] + h[j] - b;
      if (a <= x[j] && b >= x[j] + h[j]) ans += (ld) tmp;
      else if (x[j] <= a && b <= x[j] + h[j]) ans += (ld) tmp * (A * A * A - B * B * B) / (h[j] * h[j] * h[j]);
      else if (x[j] <= a && a <= x[j] + h[j]) ans += (ld) tmp * (A * A * A) / (h[j] * h[j] * h[j]);
      else if (x[j] <= b && b <= x[j] + h[j]) ans += (ld) tmp * (h[j] * h[j] * h[j] - B * B * B) / (h[j] * h[j] * h[j]);
    }
