...
int main() {
  ...
  vector<ll> v(m);
  rep (i, m) cin >> v[i];
  vector<ll> match = {0, 2, 5, 5, 4, 5, 6, 3, 7, 6};
  ...
  rep (i, n + 1) {
    ...
    rep (j, m) {
      if (i + match[v[j]] < n + 1) {
        if (dp[i + match[v[j]]] == "INF") {
          dp[i + match[v[j]]] = dp[i] + to_string(v[j]);
          continue;
        }
        if (dp[i].size() + 1 < dp[i + match[v[j]]].size()) continue;
        else if (dp[i].size() + 1 > dp[i + match[v[j]]].size()) {
          string tmp = dp[i] + to_string(v[j]);
          dp[i + match[v[j]]] = tmp;
        } else {
          string tmp = dp[i] + to_string(v[j]);
          if (tmp > dp[i + match[v[j]]]) dp[i + match[v[j]]] = tmp;
        }
      }
    }
  }
