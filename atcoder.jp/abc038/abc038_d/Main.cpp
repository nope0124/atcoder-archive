...
int main() {
  ...
  ll n; cin >> n;
  vector<pint> v(n);
  rep (i, n) cin >> v[i].first >> v[i].second, v[i].second = 200000 - v[i].second;
  sort(ALL(v));
  rep (i, n) v[i].second = 200000 - v[i].second;
  vector<ll> dp(n + 1, 1e16);
  dp[0] = 0;
  rep (i, n) {
    dp[lower_bound(ALL(dp), v[i].second) - dp.begin()] = v[i].second;
  }
  cout << lower_bound(ALL(dp), 1e16) - dp.begin() - 1 << endl;
}