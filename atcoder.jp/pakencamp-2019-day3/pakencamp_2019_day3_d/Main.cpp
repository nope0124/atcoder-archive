...
int main() {
  ...
  ll dp[n+1][3];
  rep(i,n+1) rep(j,3) dp[i][j]=1e16;
  dp[0][0]=0;
  dp[0][1]=0;
  dp[0][2]=0;
  rep(i,n){
    chmin(dp[i+1][0],min(dp[i][1],dp[i][2])+r[i]);
    chmin(dp[i+1][1],min(dp[i][0],dp[i][2])+b[i]);
    chmin(dp[i+1][2],min(dp[i][1],dp[i][0])+w[i]);
  }
