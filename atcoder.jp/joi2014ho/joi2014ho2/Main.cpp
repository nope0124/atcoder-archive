...
int main() {
  ...
  ll n,m;
  cin>>n>>m;
  vector<ll> v(n),weight(m),value(m);
  rep(i,n) cin>>v[i];
  rep(i,m) cin>>weight[i]>>value[i];
  ll dp[m+1][n+1];
  rep(i,m+1) rep(j,n+1) dp[i][j]=1e16;
  sort(ALL(v),greater<ll>());
  dp[0][0]=0;
  rep(i,m){
    rep(j,n+1){
      chmin(dp[i+1][min(j+weight[i],(ll)n)],dp[i][j]+value[i]);
      chmin(dp[i+1][j],dp[i][j]);
    }
    rep(j,n) chmin(dp[i+1][j],dp[i+1][j+1]);
  }
  vector<ll> sum=acc_sum(v);
  ll ans=0;
  rep(i,n+1) chmax(ans,sum[i]-dp[m][i]);
  cout<<ans<<endl;
}