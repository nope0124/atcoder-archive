...
int main() {
  ...
  ll ans=0;
  rep(i,h-1) rep(j,w-1){
    if(v[i][j]==s[0][0] && v[i][j+1]==s[0][1] && v[i+1][j]==s[1][0] && v[i+1][j+1]==s[1][1]){
      sum[i][j]=1;
      ans++;
    }
  }
  ...
  rep(i,h){
    rep(j,w){
      ...
      rep(k,4){
        ll ni=i-1+k%2;
        ll nj=j-1+k/2;
        if(ni<0 || h<=ni+1 || nj<0 || w<=nj+1) continue;
        if(v[ni][nj]==s[0][0] && v[ni][nj+1]==s[0][1] && v[ni+1][nj]==s[1][0] && v[ni+1][nj+1]==s[1][1]){
          if(sum[ni][nj]==0){
            tmp++;
          }
        }else{
