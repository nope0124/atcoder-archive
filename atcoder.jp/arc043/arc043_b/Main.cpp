...
int main() {
  ...
  sort(ALL(v));
  mint ans = 0;
  vector<ll> search(n, 0);
  rep (i, n) {
    search[i] = v.end() - lower_bound(ALL(v), v[i] * 2);
  }
  vector<ll> sum = acc_sum(search);
  vector<ll> nsearch(n, 0);
  rep (i, n) nsearch[i] = sum[n] - sum[n - search[i]];
  vector<ll> nsum = acc_sum(nsearch);
  rep (i, n) {
    ans += nsum[n] - nsum[n - search[i]];
  }
  cout << ans << endl;
}