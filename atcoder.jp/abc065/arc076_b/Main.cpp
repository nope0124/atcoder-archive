...
int main() {
  ...
  rep(i,n){
    ll x,y;cin>>x>>y;
    xv.push_back(pint(i,x));
    yv.push_back(pint(i,y));
  }
  sort(ALL(xv),cmp);
  sort(ALL(yv),cmp);
  vector<edge> v;
  rep(i,n-1){
    v.push_back(edge{xv[i].first,xv[i+1].first,abs(xv[i].second-xv[i+1].second)});
    v.push_back(edge{yv[i].first,yv[i+1].first,abs(yv[i].second-yv[i+1].second)});
  }
  sort(ALL(v),comp);
  UnionFind uf(n);
  ll ans=0;
