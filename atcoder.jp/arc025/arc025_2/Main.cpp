...
/* 二次元累積和全探索(最大値、面積) */
ll two_acc_sum_search(vector<vector<ll>> v,vector<vector<ll>> vv){
  // O(1/4*h*h*w*w)
  ll ans=0;
  ll h=v.size()-1;
  ll w=v[0].size()-1;
  rep(i,h) for(ll ii=i;ii<h;ii++){
    rep(j,w) for(ll jj=j;jj<w;jj++){
      if(v[ii+1][jj+1]+v[i][j]-v[ii+1][j]-v[i][jj+1]==vv[ii+1][jj+1]+vv[i][j]-vv[ii+1][j]-vv[i][jj+1]) chmax(ans,(ii+1-i)*(jj+1-j));
    }
  }
  return ans;
}
...
int main() {
  ...
  vector<vector<ll>> B=v,W=v;
  rep(i,h) rep(j,w){
    if((i+j)%2==0) B[i][j]=0;
    else W[i][j]=0;
  }
  vector<vector<ll>> sumB=two_acc_sum(B);
  vector<vector<ll>> sumW=two_acc_sum(W);
  cout<<two_acc_sum_search(sumB,sumW)<<endl;
}