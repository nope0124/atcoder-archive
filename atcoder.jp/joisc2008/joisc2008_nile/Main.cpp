...
constexpr int INF = 1e9;
 ...
 int main() {
     ...
     rep(i,370) rep(j,3010) rep(k,3) dp[i][j][k]=INF;
     rep(i,N)dp[0][i][0] = 0;
     ...
     for (int i = 0; i < D; i++) {
         ...
         for (int j = 0; j < N; j++) {
             for (int k = 0; k < 3; k++) {
                 if (k == 0) {
                    chmin(dp[(i + 1)][j][k + 1], dp[i][j][k] + a[i][j] * 9 / 10);
                 }
                 else if (k == 1) {
                    chmin(dp[(i + 1) ][j][k + 1], dp[i][j][k] + a[i][j] * 7 / 10);
                 }
                 else if (k == 2) {
                    chmin(dp[(i + 1)][j][k], dp[i][j][k] + a[i][j] * 7 / 10);
                 }
             }
         }
     }
     ...
     int ans = INT_MAX;
     for (int i = 0; i < N; i++) for (int j = 0; j < 3; j++) chmin(ans,dp[D ][i][j]);
