...
vector<ll> v;
vector<ll> tv={1,3,5,7,9};
string s;
ll n;
void dfs(vector<bool> used,map<char,ll> mp,ll x,ll ni){
  if(ni==n){
    v.push_back(x);
    return;
  }
  if(mp[s[ni]]!=0){
    dfs(used,mp,x*10+mp[s[ni]],ni+1);
  }else{
    rep(i,5){
      map<char,ll> tmp=mp;
      vector<bool> tused=used;
      if(tused[i]) continue;
      tused[i]=true;
      tmp[s[ni]]=tv[i];
      dfs(tused,tmp,x*10+tv[i],ni+1);
    }
  }
}
/*↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
