...
/* 二次元累積和全探索(最大値、面積) */
vector<ll> two_acc_sum_search(vector<vector<ll>> v){
  // O(1/4*h*h*w*w)
  ...
  rep(i,h) for(ll ii=i;ii<h;ii++){
    rep(j,w) for(ll jj=j;jj<w;jj++){
      chmax(ans[(ii+1-i)*(jj+1-j)],v[ii+1][jj+1]+v[i][j]-v[ii+1][j]-v[i][jj+1]);
    }
  }
  return ans;
}
...
int main() {
  ...
  vector<vector<ll>> sum=two_acc_sum(v);
  vector<ll> ans=two_acc_sum_search(sum);
  rep(i,ans.size()-1) if(ans[i+1]<ans[i]) ans[i+1]=ans[i];
  ll q;cin>>q;
  rep(i,q){
    ll tmp;cin>>tmp;
    cout<<ans[tmp]<<endl;
