...
int main() {
  ...
  ll na=upper_bound(ALL(asum),k)-asum.begin();
  ll tmpk=k-asum[na-1];
  ll nb=upper_bound(ALL(bsum),tmpk)-bsum.begin();
  if(na+nb==n+m){
    cout<<n+m<<endl;
    return 0;
  }
  chmax(ans,na+nb);
  ll tt=upper_bound(ALL(bsum),k)-bsum.begin();
  chmax(ans,tt);
  rep(i,na){
    ll t=k-asum[i];
    ll nnb=upper_bound(ALL(bsum),t)-bsum.begin();
    chmax(ans,nnb+i+1);
  }
  cout<<ans<<endl;
}