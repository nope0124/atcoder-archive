...
int main() {
  ...
  rep(i,k){
    ll x,y,value;cin>>x>>y>>value;x--;y--;
    v[x][y]=value;
  }
  rep(i,h+1){
    rep(j,w+1){
  ...
  rep(i,h){
    rep(j,w){
      rep(k,3){
        chmax(dp[i][j+1][k+1],dp[i][j+1][k]); // いらないかもしれない
        chmax(dp[i][j+1][k+1],dp[i][j][k+1]);
        if(k==0) chmax(dp[i][j+1][k+1],dp[i][j+1][k]+v[i][j]);
        else chmax(dp[i][j+1][k+1],dp[i][j][k]+v[i][j]);
      }
      chmax(dp[i+1][j+1][0],dp[i][j+1][3]);
    }
  }
  cout<<dp[h-1][w][3]<<endl;
}