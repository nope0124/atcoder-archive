...
vector<ll> g[1000000];
vector<ll> dist(1000000,0),seen(1000000,0);

ll dfs(ll now){
  if(seen[now]==2) return dist[now];
  seen[now]=1;
  for(auto c:g[now]){
    if(seen[c]==1){
      cout<<-1<<endl;
...
int main() {
  ...
  rep(i,n){
    rep(j,n-1){
      ll ni=i+1;
      ll nj=v[i][j];
      w[i][j]=max(ni,nj)-((n-min(ni,nj))*(n-min(ni,nj)-1)/2)+(n*n-3*n-2)/2;
    }
  }
  rep(i,n){
    rep(j,n-2){
  ...
  ll ans=-1;
  vector<bool> flag(n*(n-1)/2,false);
  rep(i,n*(n-1)/2){
    rep(j,g[i].size()){
      flag[g[i][j]]=true;
    }
  }
  rep(i,n*(n-1)/2) if(!flag[i]) chmax(ans,dfs(i)+1);
  cout<<ans<<endl;
}