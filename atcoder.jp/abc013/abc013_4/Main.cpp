...
void dis(vector<ll> v){ rep(i,v.size()) cout<<v[i]+1<<endl; }
...
int main() {
  ...
  rep(i,n) v[i]=i;
  reverse(ALL(a));
  rep(i,m) swap(v[a[i]-1],v[a[i]]);
  vector<vector<ll>> doubling(60,vector<ll>(n));
  rep(i,n) doubling[0][i]=v[i];
  rep(k,59) rep(i,n) doubling[k+1][i]=doubling[k][doubling[k][i]];
  ll ans=0;
  rep(i,n){
    ll ans=i;
    rep(j,60) if(d&(1LL<<j)) ans=doubling[j][ans];
    cout<<ans+1<<endl;
  }
}