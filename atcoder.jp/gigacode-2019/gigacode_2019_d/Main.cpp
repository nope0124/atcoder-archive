...
/* 二次元累積和全探索(最大値、面積) */
ll h,w,k,vv;
ll two_acc_sum_search(vector<vector<ll>> v){
  ll ans=0;
  ll h=v.size()-1;
  ll w=v[0].size()-1;
  rep(i,h) for(ll ii=i;ii<h;ii++){
    rep(j,w) for(ll jj=j;jj<w;jj++){
      ll tmp=v[ii+1][jj+1]+v[i][j]-v[ii+1][j]-v[i][jj+1];
      if(vv>=tmp+(ii+1-i)*(jj+1-j)*k) chmax(ans,(ii+1-i)*(jj+1-j));
    }
  }
  return ans;
}
