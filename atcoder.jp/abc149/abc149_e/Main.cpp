...
ll solve(ll mid,vector<ll> v){
  ll n=v.size();
  ll ans=0;
  rep(i,n) ans+=v.end()-lower_bound(ALL(v),mid-v[i]);
  return ans;
}
/*↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
int main() {
  ...
  while(abs(ok-ng)>1){
    ll mid=(ok+ng)/2;
    if(solve(mid,v)>=k) ok=mid;
    else ng=mid;
  }
  rep(i,n){
    ll tmp=v.end()-upper_bound(ALL(v),ok-v[i]);
    ans+=tmp*v[i];
    cnt+=tmp;
    if(v.end()-upper_bound(ALL(v),ok-v[i])>=1) ans+=sum[n-tmp];
  }
  ans+=(k-cnt)*ok;
  cout<<ans<<endl;
}