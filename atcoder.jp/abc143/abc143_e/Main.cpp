...
int main() {
  ...
  rep(i,n) rep(j,n) if(i==j) v[i][j]=0,ans[i][j]=0;
  rep(i,m){
  ...
  rep(i,n) rep(j,n) if(v[i][j]<=l) ans[i][j]=1;
  rep(i,n) rep(j,n) if(i==j) ans[i][j]=0;
  rep(k,n) rep(i,n) rep(j,n) chmin(ans[i][j],ans[i][k]+ans[k][j]);
  ll q;cin>>q;
  rep(i,q){
    ll s,t;cin>>s>>t;s--;t--;
