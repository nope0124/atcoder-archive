...
int main() {
  ...
  ll n=s.size();
  ll dp[n+1][(1<<10)][2];
  rep(i,n+1) rep(bit,(1<<10)) dp[i][bit][0]=1e16,dp[i][bit][1]=-1e16;
  ll x=0;
  dp[0][0][0]=0;
  dp[0][0][1]=0;
  rep(i,n){
    ...
    rep(bit,(1<<10)){
      ll cnt=0;
      rep(j,10) if(bit&(1<<j)) cnt++;
      if(cnt>k) continue;
      rep(j,10) rep(flag,2){
        ll ni=dp[i][bit][flag]*10+j;
        if(cnt==k && !(bit&(1<<j))) continue;
        if(ni<x){
          chmax(dp[i+1][bit|(1<<j)][1],ni);
        }else{
          chmin(dp[i+1][bit|(1<<j)][0],ni);
        }
      }
    }
  }
  ll ans=1e16;
  rep(i,n+1) rep(bit,(1<<10)) rep(j,2) chmin(ans,abs(x-dp[i][bit][j]));
  cout<<ans<<endl;
}